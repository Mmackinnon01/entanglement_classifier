...
class TestReservoir(unittest.TestCase):
    ...
    def test_reservoir_spin_down_nodes_setup(self):
        ...
        self.assertTrue((res.nodes[3].init_quantum_state == spin_down).all(), "Node not initialised in spin down")

    def test_reservoir_spin_up_nodes_setup(self):
        res = Reservoir(self.factory)
        res.setupNodes(1, system_nodes=3, quantum_state=1)
        self.assertTrue((res.nodes[3].init_quantum_state == spin_up).all(), "Node not initialised in spin up")

    def test_reservoir_number_connection_setup(self):
        res = Reservoir(self.factory)
        res.setupNodes(10, system_nodes=3)
        res.setupConnections()
        self.assertEqual(len(res.connections), 9*5, "Incorrect number of connections created")
    ...
    def test_reservoir_connection_node_ids(self):
        ...
        self.assertTrue(
            (
                ...
                and
                res.connections["res_connection_34"].node2 == 4
            ),
    ...
    def test_reservoir_connection_equation(self):
        ...
        res.setupConnections()
        self.assertEqual(res.connections["res_connection_34"].calc(), 3, "Incorrect connection calc")
