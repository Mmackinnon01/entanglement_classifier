...
class TestInterface(unittest.TestCase):
    ...
    def test_interface_number_connection_setup(self):
        itf = Interface(sys_nodes=[0, 1], res_nodes=[2, 3], connectionFactory=self.factory)
        itf.setupConnections()
        self.assertEqual(len(itf.connections), 4, "Incorrect number of connections created")
    ...
    def test_interface_connection_node_ids(self):
        ...
        self.assertTrue(
            (
                ...
                and
                itf.connections["interface_connection_02"].node2 == 2
            ),
    ...
    def test_interface_connection_equation(self):
        itf = Interface(sys_nodes=[0, 1], res_nodes=[2, 3], connectionFactory=self.factory)
        itf.setupConnections()
        self.assertEqual(itf.connections["interface_connection_02"].calc(), 3, "Incorrect connection calc")